...
int main()
{
  ...
  while(i++<1){ //change this with a reasonable condition !!!
   ...
   ret = fork();
   //after fork(), code/data/heap/stack are duplicated
   //using duplicate VADs, which are private VADs
   //however, shared memory VADS are duplicated, but
   //set as shared VADs - what does this mean ??
   //when shared VADs are used and point to the same
 ...
 if(ret>0)
 {
   while(1){ //this while(1) is ok - it has been used with a clear purpose
    ...
    if(ret<0) { 
           ...
           //shmctl(id,IPC_RMID); //destroy the shared memory object !!!
