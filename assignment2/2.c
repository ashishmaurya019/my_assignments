	...
	while(i++<4)
	{
		...
		*/	continue;
		...
		if(i==1){
			...
			ret=execl("/usr/bin/firefox","firefox","firefox",NULL);
		 ...
		 if(i==2){
			...
			exit(0);}
		if(i==3){
	                 ret=execl("/usr/bin/okular","okular","ipc.pdf",NULL);
		
			 if(ret<0){perror("error in loading");exit(0);}
				 exit(0);}
		 if(i==4){
	                         ret=execl("/usr/bin/banshee-1","banshee","banshee-1","-c","-o","mul.o",NULL);
				 if(ret<0){perror("error in loading");exit(0);}
				 exit(0);}
		if(i==5)
			{
